...
impl TGAImage {
    ...
    pub fn set(&mut self, x: u16, y: u16, pixel: &TGAPixel) -> Result {
    ...
    pub fn get(&self, x: u16, y: u16) -> Option<TGAPixel>{
        if x >= self.width || y >= self.height {
            None
        } else {
            let start = self.coords_to_index(x, y);
            let pixel: TGAPixel = self.data.iter()
                .skip(start)
                .take(3)
                .collect();
            Some(pixel)
...
mod tests {
    ...
    fn creating_a_2_by_2_picture_should_result_in_rgb_repeating_4_times() {
        let image = TGAImage::new(2u16, 2u16, &TGAPixel::from_rgb(1, 2, 3));
        assert_eq!(
            image.data, vec![
                3, 2, 1,  3, 2, 1,
                3, 2, 1,  3, 2, 1,
    ...
    fn getting_a_valid_pixel_should_return_some_valid_pixel() {
        let image = TGAImage::new(3, 3, &TGAPixel::from_rgb(1, 2, 3));
        assert_eq!(image.get(1, 2), Some(TGAPixel::from_rgb(1, 2, 3)));
    }
    ...
    fn setting_a_valid_pixel_should_return_ok_and_should_set_the_right_pixel() {
        let mut image = TGAImage::new(3, 3, &TGAPixel::from_rgb(0, 0, 0));
        assert_eq!(image.set(0, 0, &TGAPixel::from_rgb(1, 1, 1)), Ok(()));
        assert_eq!(image.get(0, 0), Some(TGAPixel::from_rgb(1, 1, 1)));
    }

    #[test]
    fn setting_an_over_indexed_by_one_pixel_on_the_x_axis_should_return_err() {
        let mut image = TGAImage::new(3, 3, &TGAPixel::from_rgb(0, 0, 0));
        assert_eq!(image.set(3, 2, &TGAPixel::from_rgb(1, 1, 1)), Err(()));
    }

    #[test]
    fn setting_an_over_indexed_by_one_pixel_on_the_y_axis_should_return_err() {
        let mut image = TGAImage::new(3, 3, &TGAPixel::from_rgb(0, 0, 0));
        assert_eq!(image.set(2, 3, &TGAPixel::from_rgb(1, 1, 1)), Err(()));
    }

    #[test]
    fn setting_an_over_indexed_pixel_should_return_err() {
        let mut image = TGAImage::new(10, 10, &TGAPixel::from_rgb(0, 0, 0));
        assert_eq!(image.set(20, 20, &TGAPixel::from_rgb(1, 1, 1)), Err(()));
    }
}