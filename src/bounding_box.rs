...
impl<S: BaseNum + PartialOrd> BoundingBox2<S> {
    pub fn from_triangle(triangle: &Triangle<S>) -> BoundingBox2<S> {
        use std::cmp::Ordering::Less;
        let x_coordinate_comparator =
            |p: &&&Point3<S>, q: &&&Point3<S>| p.x.partial_cmp(&q.x).unwrap_or(Less);
        let y_coordinate_comparator =
            |p: &&&Point3<S>, q: &&&Point3<S>| p.y.partial_cmp(&q.y).unwrap_or(Less);
        ...
        let min_y_point = points.iter().min_by(y_coordinate_comparator).unwrap();
        let max_x_point = points.iter().max_by(x_coordinate_comparator).unwrap();
        let max_y_point = points.iter().max_by(y_coordinate_comparator).unwrap();
        BoundingBox2 {
